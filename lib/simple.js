...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        text: {
            get() {
                const msg = this.msg;
                // ================== NORMALIZACIÓN DE RESPUESTAS DE BOTONES ==================
                // Unificamos para que los comandos disparados por botones/listas lleguen como si el usuario escribiera el texto (ID)
                try {
                    // 1. Respuesta de botón clásico
                    if (this.mtype === 'buttonsResponseMessage') {
                        const br = this.message?.buttonsResponseMessage;
                        const id = br?.selectedButtonId || '';
                        const display = br?.selectedDisplayText || '';
                        if (id) {
                            this.buttonId = id;
                            this.buttonDisplayText = display;
                            return id; // devolvemos el id para que el handler lo trate como comando
                        }
                    }
                    // 2. Respuesta de template button
                    if (this.mtype === 'templateButtonReplyMessage') {
                        const tr = this.message?.templateButtonReplyMessage;
                        const id = tr?.selectedId || '';
                        const display = tr?.selectedDisplayText || '';
                        if (id) {
                            this.buttonId = id;
                            this.buttonDisplayText = display;
                            return id;
                    ...
                    // 3. Respuesta de lista
                    if (this.mtype === 'listResponseMessage') {
                        const lr = this.message?.listResponseMessage;
                        const id = lr?.singleSelectReply?.selectedRowId || '';
                        const title = lr?.title || '';
                        if (id) {
                            this.buttonId = id;
                            this.buttonDisplayText = title;
                            return id;
                        }
                    }
                    // 4. Respuesta de mensaje interactivo (native flow)
                    if (this.mtype === 'interactiveResponseMessage') {
                        const ir = this.message?.interactiveResponseMessage?.nativeFlowResponseMessage;
                        if (ir?.paramsJson) {
                            try {
                                const params = JSON.parse(ir.paramsJson);
                                const id = params.id || '';
                                const display = params.text || params.title || '';
                                if (id) {
                                    this.buttonId = id;
                                    this.buttonDisplayText = display;
                                    return id;
                                ...
                                console.log('[Button Handler] JSON inválido en interactiveResponseMessage:', e.message);
                            }
                        }
                    }
                } catch (e) {
                    console.log('[Button Handler] Error general:', e.message);
                }
                // ==========================================================================
